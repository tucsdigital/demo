...
import { generateMenusConfig } from "@/lib/menus-dynamic";
...
const ModuleSidebar = ({ trans }) => {
  // Usar menús dinámicos basados en módulos habilitados
  const { enabledModules } = useLicense();
  const dynamicMenus = enabledModules.length > 0 
    ? generateMenusConfig(enabledModules)
    : menusConfig;
  
  const menus = dynamicMenus?.sidebarNav?.modern || menusConfig?.sidebarNav?.modern || [];
